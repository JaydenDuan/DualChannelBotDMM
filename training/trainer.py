...
class Trainer:
    ...
    def train(self, train_loader, val_loader, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            if val_metrics['accuracy'] > best_val_acc:
                ...
                patience_counter = 0
                self.save_checkpoint(epoch, val_metrics, best_model_path)
            else:
                patience_counter += 1
    ...
    def save_checkpoint(self, epoch, metrics, path=None):
        if path is None:
            path = os.path.join(self.save_dir, f'checkpoint_epoch_{epoch}.pt')
        torch.save(self.model.state_dict(), path)
        logger.info(f"Model saved to {path} (Val Acc: {metrics['accuracy']:.4f})")
